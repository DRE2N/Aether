...
@QLoadableDoc(
        value = "hide_mob",
        description = "Hides tagged mobs. They have to be set to `instanceable` for this to work.",
        shortExample = "hide_mob: tag=bandit; mode=event_in_range",
        longExample = {
                "hide_mob:",
                "  tag: bandit",
        }
)
public class HideMobAction extends QBaseAction {
        ...
        try {
            ...
            for (AetherBaseMob mob : mobs) {
                if (quester instanceof QEvent event) {
                    switch (mode) {
                        case EVENT_IN_RANGE -> execute(quester, player -> player.getPlayer().hideEntity(Aether.getInstance(), mob.getBukkitLivingEntity()));
                        case EVENT_PARTICIPANTS -> {
                            for (QPlayer player : event.getParticipants().keySet()) {
                                player.getPlayer().hideEntity(Aether.getInstance(), mob.getBukkitLivingEntity());
                            }
                        }
                        case ALL -> {
                            for (Player player : Bukkit.getOnlinePlayers()) {
                                player.hideEntity(Aether.getInstance(), mob.getBukkitLivingEntity());
                            }
                        }
                        default -> {
                ...
                if (quester instanceof QPlayer qPlayer) {
                    switch (mode) {
                        case SELF -> qPlayer.getPlayer().hideEntity(Aether.getInstance(), mob.getBukkitLivingEntity());
                        case GROUP -> {
            ...
            FriendlyError error = new FriendlyError(id,"Failed to hide mob", e.getMessage(), "Mob tag: " + tag).addStacktrace(e.getStackTrace());
